...
int check(char *host){
	...
	printf("SEARCHING diff time : %lf(sec)\n", diff);
	return ans;
}
...
int main(int argc, char **argv)
{
	...
	sprintf(command, "top -p %d -n 1 -b > tmp1.txt", pid);
	system(command);
	...
	sprintf(command, "top -p %d -n 1 -b > tmp2.txt", pid);
	system(command);

	FILE *file1, *file2;
	char line[256];
	file1 = fopen("tmp1.txt", "r");
	file2 = fopen("tmp2.txt", "r");
